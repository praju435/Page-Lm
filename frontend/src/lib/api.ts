...
export function connectExamStream(runId: string, onEvent: (ev: ExamEvent) => void) {
  ...
  const ws = new WebSocket(url)
  const chunks = new Map<string, { idx: number; data: string }[]>()

  ws.onmessage = (m) => {
    try {
      const msg = JSON.parse(m.data as string) as any
      console.log("[connectExamStream] received message:", msg.type, msg)
      
      // Handle chunked messages (exam.chunk, exam.done)
      if (msg.type === "exam.chunk") {
        console.log(`[connectExamStream] chunk ${msg.idx}/${msg.total} for id ${msg.id}`)
        if (!chunks.has(msg.id)) chunks.set(msg.id, [])
        chunks.get(msg.id)!.push({ idx: msg.idx, data: msg.data })
      } else if (msg.type === "exam.done") {
        console.log(`[connectExamStream] exam.done for id ${msg.id}, total chunks: ${chunks.get(msg.id)?.length || 0}`)
        const parts = chunks.get(msg.id)
        if (parts) {
          parts.sort((a, b) => a.idx - b.idx)
          const fullData = parts.map(p => p.data).join("")
          console.log(`[connectExamStream] reassembled ${fullData.length} bytes`)
          let decoded: any
          try {
            decoded = JSON.parse(fullData)
          } catch (parseErr) {
            console.error("[connectExamStream] JSON parse failed:", parseErr)
            throw parseErr
          }
          console.log("[connectExamStream] emitting exam event with", decoded.payload?.length || 0, "questions")
          onEvent({ type: "exam", examId: decoded.examId, payload: decoded.payload })
          chunks.delete(msg.id)
        } else {
          console.warn(`[connectExamStream] no chunks found for id ${msg.id}`)
        }
      } else {
        // Other event types (ready, phase, done, error)
        console.log("[connectExamStream] forwarding event:", msg.type)
        onEvent(msg as ExamEvent)
      }
    } catch (err) {
      console.error("[connectExamStream] Error processing message:", err)
    }
  }
  ws.onerror = () => {
    console.error("[connectExamStream] WebSocket error")
    onEvent({ type: "error", error: "stream_error" })
  }
  return { ws, close: () => { try { ws.close() } catch { } } }
}
