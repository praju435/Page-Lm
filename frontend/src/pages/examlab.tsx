...
export default function ExamLabs() {
  ...
  async function start(id: string) {
    ...
    try {
      ...
      console.log("[examlab] startExam returned runId:", runId)
      const { close } = connectExamStream(runId, (ev: ExamEvent) => {
        console.log("[examlab] received event:", ev)
        if (ev.type === "exam") {
          const arr = Array.isArray(ev.payload) ? ev.payload : []
          console.log("[examlab] received exam with", arr.length, "questions")
          setQs(arr.map((q: any) => ({ ...q, correct: Math.max(0, q.correct - 1) })))
          setIdx(0)
          setConnecting(false)
        }
        if (ev.type === "done" || ev.type === "error") {
          console.log("[examlab] exam done or error:", ev.type)
          setConnecting(false)
        }
      })
      closeRef.current = close
    } catch (e) {
      console.error("[examlab] error:", e)
      setConnecting(false)
    }
  }
